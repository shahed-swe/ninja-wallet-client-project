...
async function upgradeToPremium() {
  try {
    ...
    const userBefore = await fetch('http://localhost:5001/api/users/me', {
      headers: {
        Cookie: cookies
      }
    }).then(res => res.json());
    ...
    await fetch('http://localhost:5001/api/execute-sql', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Cookie: cookies
    ...
    // Verify the update
    const userAfter = await fetch('http://localhost:5001/api/users/me', {
      headers: {
        Cookie: cookies
      }
    }).then(res => res.json());
